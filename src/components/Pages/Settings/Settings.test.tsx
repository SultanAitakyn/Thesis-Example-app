...
it("Should show errors", async () => {
  await act(async () => {
    store.dispatch(
      updateErrors({ "email or password": ["is invalid", "is empty"] })
    );
  });
...
it("Should disable fields during update and enabled afterwards with errors", async () => {
  mockedUpdateSettings.mockResolvedValueOnce(
    Err({ "email or password": ["is invalid2", "is empty3"] })
  );
...
it("Should redirect to home if settings update succeeds", async () => {
  ...
  expect(store.getState().app.user.unwrap().email).toMatch(
    "jake@jake.jakesettings"
  );
});
